...
-- 사원테이블의 사번, 사원명, 성별, 입사일, 급여 컬럼을 조회한 한글 컬럼명으로 출력
 SELECT EMP_ID AS 사번, EMP_NAME AS "사 원 명", GENDER AS 성별, HIRE_DATE AS 입사일, SALARY AS 급여 
 FROM EMPLOYEE;
...
-- 사번이 S5005인 사원의 사원명, 성별, 입사일, 부서명, 부서아이디, 이메일, 급여를 조회
...
-- AS와 *를 같이 사용 불가
-- SELECT EMP_ID AS '사원번호', * FROM EMPLOYEE WHERE DEPT_ID = 'SYS';
...
SELECT EMP_ID, EMP_NAME, DEPT_ID, SALARY, SALARY*1.2 AS BONUS
FROM EMPLOYEE
WHERE RETIRE_DATE IS NULL;
...
-- HIRE_DATE, SALARY 컬럼은 '입사일', '급여' 별칭으로 컬럼 리스트 생성 후 정렬
SELECT EMP_ID, EMP_NAME, GENDER, HIRE_DATE AS 입사일, RETIRE_DATE, DEPT_ID, PHONE, EMAIL, SALARY AS 급여
FROM EMPLOYEE
WHERE DEPT_ID = "SYS"
...
select floor(123.456), truncate(123.456, 0), truncate(123.456, 1), truncate(123.456, 2) from dual;
...
-- 정수 4자리(0 ~ 9999) 난수 발생, 소수점 2자리까지 출력
select truncate(rand()*10000, 2) as number from dual;

-- (4) mod(숫자, 나누는 수) : 나머지 함수
select mod(123, 2) as odd, mod(123, 1) as even from dual;
...
-- 사번, 사원명, S001(홍길동) 형식으로 컬럼을 생성하여 조회
...
select emp_id, emp_name, hire_date, substring(hire_date, 1, 4) as year, substring(hire_date, 6, 2) as month, substring(hire_date, 9, 2) as day, salary
from employee;
...
-- where left(hire_date, 4) >= '2015' and left(hire_date, 4) <= '2017';
...
select 	'12345' as string,
			cast('12345' as signed integer) as cast_int,		-- 정수
			cast('12345' as unsigned integer) as cast_int,	-- 부호가 붙지않은 정수
            cast('12345' as decimal(10, 2)) as cast_decimal	-- 실수(소수부분을 포함한 실수의 총 자리 수, 소숫점 자리 수)
from dual;
...
-- select * from employee;		-- 모든 데이터 타입의 구분은 '-'로 구분(default)
...
select 	emp_id, 
			emp_name, 
            -- replace(cast(hire_date as char), '-', '/') as hire_date,	-- 구버전
            replace(hire_date, '-', '/') as hire_date, 							-- 현버전
...
/****************************************************************
    ...
    ** 그룹함수는 그룹핑이 가능한 반복된 데이터를 가진 컬럼과 사용하는 것이 좋음
****************************************************************/
...
select min(hire_date) as '가장 빠른 입사자', max(hire_date) as '가장 늦은 입사자'
from employee;
...
select 	dept_id as 부서별, 
			sum(salary) as '부서별 총 급여', 
            avg(salary) as '부서별 평균 급여', 
            count(*) as '부서별 총원', 
            max(salary) as '부서별 최대급여', 
			min(salary) as '부서별 최소급여'
from employee
group by dept_id;
...
select 	left(hire_date, 4) as 연도, 
			count(*) as '사원 수',
			format(sum(salary), 0) as '총 급여', 
			format(avg(salary), 0) as '평균 급여', 
            format(max(salary), 0) as '최대급여', 
			format(min(salary), 0) as '최소급여'
from employee
...
-- cross join
select *
from employee, department;	-- employee(20) * department(7) = 140

select count(*)
from employee cross join department;	-- ANSI SQL 방식

--
select count(*) from vacation;	-- vacation(102)

-- 사원, 부서, 휴가 테이블 cross join : 20 * 7 * 102 = 14280 -> 이런 JOIN이 있다는 것을 알아두기(실제로 이렇게 쓰면 데이터베이스 다운됨;)
...
select *	-- 19개 (1개 유실)
from employee e, department d, unit u	-- 별칭 선언 가능
...
-- 사원테이블, 부서테이블, 본부테이블, 휴가테이블 inner join : oracle
select *
...
-- select *
-- from employee e inner join department d inner join unit u inner join vacation v 
-- on e.dept_id = d.dept_id and d.unit_id = u.unit_id and e.emp_id = v.emp_id
-- order by e.emp_id;
...
from employee e, department d
where 	e.dept_id = d.dept_id 
			and d.dept_name = '영업';		-- 조건절로 영업부를 필터링
...
from employee e, department d, unit u, vacation v
where 	e.emp_id = v.emp_id and e.dept_id = d.dept_id and d.unit_id = u.unit_id
			and d.dept_name = '영업' and v.reason = '두통';
...
select dept_id from department where dept_name = '정보시스템';		-- 서브쿼리를 먼저 작성해서 정확한 지 확인 후 대입하기
...
select dept_name from department where dept_name = '정보시스템';	-- 스칼라 서브쿼리를 먼저 작성해서 정확한 지 확인 후 대입하기
...
-- 2) 1번의 실행 결과와 employee 테이블 조인하여 사원아이디, 사원명, 급여, 부서아이디, 부서명, 부서별 총급여, 평균급여 출력
select e.emp_id, e.emp_name, e.salary, e.dept_id, d.dept_name, t.avg
...
/**********************************************************************************
	...
	** 실행결과 컬럼이 동일(컬럼명, 데이터타입) 해야함
**********************************************************************************/
-- 영업부, 정보시스템 부서의 사원아이디, 사원명, 급여, 부서아이디 조회
-- union : 영업 부서 사원들이 한번만 출력 (중복되는 로우 제거하고 출력)
...
-- union all : 영업 부서 사원들이 (실행한 횟수만큼) 중복되어 출력
select emp_id, emp_name, salary, dept_id
from employee
where dept_id = (select dept_id from department where dept_name = '영업')
union all
...
/*****************************************************************************************
	...
	** 뷰 생성 시 권한을 할당받아야 함 - mysql, maria  제외
*****************************************************************************************/
select *
...
show tables;	-- emp 테이블이 생성된 것을 확인 가능
...
show tables;	-- emp 테이블이 삭제된 것을 확인 가능
...
show tables;	-- employee 테이블을 복제한 emp 테이블이 생성된 것을 확인 가능
desc employee;
desc emp;		-- Key값 같은 제약사항까지는 복제하지 못함, 따로 추가해주던가 해야함
...
insert into emp(ename, emp_id, gender, salary, hire_date)	-- 데이터 타입이 맞아야 실행 가능, 예) salary에 now() -> 오류
	values('s001', '홍길동', 'm', 1000, null) ;	-- null이 허용된 상태만 null 삽입이 가능

insert into emp(emp_id)
	values('s002');	-- null을 허용하는 컬럼은 값을 주지 않으면 default로 null이 들어감
...
truncate table emp;
select * from emp;	-- 테이블 구조는 그대로지만 데이터는 삭제된 것을 확인 가능
drop table emp;
show tables;
...
create table emp2 (
	emp_id		int				auto_increment	primary key,		-- primary key : unique + not null (중복도 안되고, 비어있어도 안됨)
    ename		varchar(10)	not null,
    gender		char(1)			not null,
    hire_date	date,
    salary		int
...
show tables;	-- emp2 테이블 생성된 것을 확인 가능
desc emp2;		-- emp2 테이블 구조 확인 가능
insert into emp2(ename, gender, hire_date, salary)	-- emp_id는 auto_increment이기 때문에 빼야함
...
alter table emp
		modify column phone char(10) null;	-- 저장된 데이터보다 크기가 작으면 에러 발생 : 데이터 유실 위험 발생!!, 예) char(13) -> char(10)
																-- 저장된 데이터보다 크기가 크면 문제없이 실행 가능 , 예) char(10) -> char(20) 
-- phone 컬럼 삭제
alter table emp
		drop column phone;
...
rollback;	-- Auto commit 버튼 끄고 실행하면 롤백해서 삭제했던 데이터 복구가 가능한 것을 확인 가능
...
insert into emp_const2(emp_id, emp_name, hire_date, salary)
	values('s001', '홍길동', now(), 1000);	-- hire_date는 date 타입인데 now() 이용하면 시분초까지 줘서 효율성이 떨어짐
...
update emp_const2
    ...
    where emp_id = 's001';	-- pk로 주는 것을 권장
select * from emp_const2;
...
-- department 테이블의 복사본 : dept, employee 테이블의 복사본 : emp
create table dept
as
select * from department
	where unit_id is not null;	-- unit_id 값이 있는 것들만 복제
    
show tables;	-- dept 테이블 생성된 것을 확인 가능
desc dept;		-- 복사하면 primary key까진 복사하진 않음
select * from dept;
-- dept_id 컬럼에 primary key 제약 추가
alter table dept
	add constraint pk_dept_id primary key(dept_id);
select * from information_schema.table_constraints
	where table_name = 'dept';	-- primary key가 제대로 추가되었는지 확인
desc dept;	-- dept_id에 primary key 추가된 것을 확인 가능
...
show tables;	-- emp 테이블 생성된 것을 확인 가능
desc emp;
select * from emp;
...
select * from information_schema.table_constraints
	where table_name = 'emp';	-- primary key가 제대로 추가되었는지 확인
desc emp;	-- emp_id에 primary key 추가된 것을 확인 가능
...
alter table emp
	add constraint fk_dept_id foreign key(dept_id)
		references dept(dept_id);	-- 참조하는 부분 추가 작성
...
insert into emp
	values('S0002', '홍길동', null, 'M', curdate(), null, 'SYS', '010-1234-2345', 'hong@test.com', null);	-- 존재하지 않는 dept_id 기입 시(예: ABC) 에러 발생
select * from emp;
...
-- 교수 데이터 추가
insert into professor(pname, sid, pdate) values('스미스', 1, now());
insert into professor(pname, sid, pdate) values('홍홍', 3, now());
insert into professor(pname, sid, pdate) values('김철수', 4, now());
select * from professor;

-- 홍길동 학생이 수강하는 과목명을 조회 (오라클 조인, ANSI SQL 조인, 서브쿼리)
select s.sname
from subject s, student st
where s.sid = st.sid and st.sname = '홍길동';

select s.sname
from subject s inner join student st on s.sid = st.sid
where st.sname = '홍길동';
...
select s.sname as 과목명, st.sname as 학생명
from subject s, student st
where s.sid = st.sid and st.sname = '홍길동';

-- 스미스 교수가 강의하는 과목을 조회 (오라클 조인, ANSI SQL 조인, 서브쿼리)
select s.sname
from subject s, professor p
where s.sid = p.sid and p.pname = '스미스';
...
select sname
from subject
where sid = (select sid from professor where pname = '스미스');

-- java, 안중근 교수 추가
insert into professor(pname, sid, pdate) values('안중근', 1, now());
select * from professor;

-- java 수업을 강의하는 모든 교수 조회
select p.pname 
from professor p, subject s
where p.sid = s.sid and s.sname = 'java';

select p.pname
from professor p inner join subject s on p.sid = s.sid
where s.sname = 'java';
...
-- 과목아이디, 과목명, 교수명, 학생명
select s.sid, s.sname, p.pname, st.sname
from subject s, professor p, student st
where s.sid = p.sid and s.sid = st.sid and s.sname = 'java';
...
create table orderitem(
    ...
    constraint fk_order_id_order foreign key(order_id)
		references `order`(order_id),
	constraint fk_product_id_product foreign key(product_id)
...
show tables;
select * from information_schema.table_constraints
	where table_name in ('member', 'product', 'order', 'orderitem');
...
desc product;
-- insert into product(name, price)	values('모니터', 1000);
-- insert into product(name, price)	values('키보드', 2000);
-- insert into product(name, price)	values('마우스', 2500);
insert into product(name, price)	values('모니터', 1000), ('키보드', 2000), ('마우스', 2500);
select * from product;

desc `order`;
insert into `order`(member_id, order_date) values(1, '2024-06-25'), (2, '2025-01-25');
select * from `order`;

desc orderitem;
insert into orderitem(order_id, product_id, quantity, unit_price) values(1, 1, 2, 1000);
insert into orderitem(order_id, product_id, quantity, unit_price) values(2, 2, 1, 2000);
insert into orderitem(order_id, product_id, quantity, unit_price) values(3, 2, 5, 2000);
insert into orderitem(order_id, product_id, quantity, unit_price) values(4, 3, 2, 2500);
select * from orderitem;

-- 홍길동 고객의 고객명, 이메일, 가입날짜, 주문날짜를 조회
-- 주문날짜는 년, 월, 일로만 출력
select m.name, m.email, date(m.created_at), date(o.order_date)
...
--
insert into product(name, price)	values('리모컨', 3000), ('USB', 2000);
select * from product;
...
where m.member_id = o.member_id and o.order_id = oi.order_id and oi.product_id = p.product_id;
...
from (select distinct m.name, m.created_at, o.order_date, oi.quantity, oi.product_id
		...
		where m.member_id = o.member_id and o.order_id = oi.order_id) t1 right outer join product p on t1.product_id = p.product_id;
...
select 
	...
	emp_id, emp_name, concat(left(hire_date, 4), '년') as hire_date, phone, email, concat(format(salary, 0), '만원') as salary
from employee;    

-- rno 행번호 추가, 주문날짜(년, 월, 일), 가격(소수점 생략, 3자리 구분)
select 
	row_number() over() as rno,	-- 기준이 없을 땐 비워도 됨
	t1.name, t1.created_at, left(t1.order_date, 10) as order_date, t1.quantity, p.name, format(floor(p.price), 0) as price
from (select distinct m.name, m.created_at, o.order_date, oi.quantity, oi.product_id
		from member m, `order` o, orderitem oi
		where m.member_id = o.member_id and o.order_id = oi.order_id) t1 right outer join product p on t1.product_id = p.product_id;
...
before insert	 on trg_member -- 테이블명	-- before : insert가 실행되기 전에 만들어짐
for each row
begin
...
delimiter ;
/******************************************************************************************/
select * from information_schema.triggers;		-- 트리거 등록 되었는지 확인
select * from trg_member;
...
before insert	 on employee_stru -- 테이블명	-- before : insert가 실행되기 전에 만들어짐
for each row
begin
...
/******************************************************************************************/
select * from information_schema.triggers;		-- 트리거 등록 되었는지 확인
desc employee_stru;		-- 구조, 키 확인

insert into employee_stru(emp_name, gender, hire_date, dept_id, phone, email, salary) values('홍길동', 'M', curdate(), 'SYS', '010-1234-1234', 'hong@test.com', 1000);
select * from employee_stru;

-- 초기화
-- drop table employee_stru;
...
-- ACC 부서 삭제
delete from dept where dept_id = 'ACC';	-- emp의 고소해 사원이 참조이어서 삭제 불가능!
...
insert into board(title, content, bdate) values('test', 'test', curdate());
insert into reply(content, bid, rdate) values('reply test', 2, curdate());
...
-- **** 오라클 데이터베이스에서는 무조건 트리거 실행 가능!!
-- **** innoDB 형식의 데이터베이스인 mysql, maria는 트리거 실행 불가능!!
-- 이유는 innoDB 형식은 트리거 실행 전 참조관계를 먼저 체크하여 에러 발생 시킴!!

select * from information_schema.triggers;
/******************************************************************************************/
-- dept 테이블의 row 삭제 시(dept_id 컬럼 포함), 참조하는 emp 테이블의 dept_id에 null값 업데이트
delimiter $$
...
-- 사원 테이블의 급여 변경 시 로그 저장, old.salary(기존급여), new.salary(새로운급여)
	if old.salary <> new.salary	then	-- old.salary와 new.salary의 값이 다르다면
		insert into salary_log(emp_id, old_salary, new_salary, change_date)	-- 로그 저장
